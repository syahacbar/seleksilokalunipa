...
var optionsasalslta = {
    url: function(phrase) {
        return "<?php echo site_url('pendaftar/asalslta/')?>";
    },
    getValue: "asalslta",
    list: {
        match: {
...
var optionsjenjangslta = {
    url: function(phrase) {
        return "<?php echo site_url('pendaftar/jenjangslta/')?>";
    },
...
var optionsjurusanslta = {
    url: function(phrase) {
        return "<?php echo site_url('pendaftar/jurusanslta/')?>";
    },
...
function add_record()
{
    ...
    $("#error_nfigural").html('');
     ...
     $.ajax({
        url : "<?php echo site_url('pendaftar/ajax_get/')?>",
        type: "GET",
        dataType: "JSON",
...
function edit_record(id)
{
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            $('[name="nnumerik"]').val(data.nnumerik);
            $('[name="ninggris"]').val(data.ninggris);
            $('[name="nfigural"]').val(data.nfigural);
            $('[name="status"]').val(data.status);
            $('[name="tahunakademik"]').val(data.tahunakademik);
    ...
    $.ajax({
        ...
        success: function(data)
        {
            if (data.hasil !== "sukses") {
                ...
                $("#error_nnumerik").html(data.error.nnumerik);
                $("#error_ninggris").html(data.error.ninggris);
                $("#error_nfigural").html(data.error.nfigural);
            }
...
function upload_file()
{
        $.ajax({
            beforeSend:function()
            {
                ...
                ajax.open("POST", "<?php echo site_url('pendaftar/importexcel')?>", true);
                ajax.send(formdata);
            },
            success:function(data)
            {   if(data.status){
                ...
                <form action="#" id="form-pendaftar" class="form-horizontal">
                    <div class="form-body">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-3">
                                <input name="nnumerik" placeholder="Nilai Numerik " class="form-control" type="number">
                                <span class="text-danger" id="error_nnumerik"></span>
                            </div>
                            <div class="col-md-3">
                                <input name="ninggris" placeholder="Nilai Inggris" class="form-control" type="number">
                                <span class="text-danger" id="error_ninggris"></span>
                            </div>
                            <div class="col-md-3">
                ...
                <div class="row">
                    <div class="modal-body">
                        <div class="col-md-12">
                            ...
                            <a class="btn btn-xs btn-info" target="blank" href="<?=site_url('public/format_import_pendaftar.xls')?>"><i class="glyphicon glyphicon-import"></i>Downnload Format Excel</a>
                        </div>
                    </div>
                </div>
